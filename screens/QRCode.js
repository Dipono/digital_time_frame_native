...
function QRCode() {
  ...
  function setWeeklyAverage(currentDate, weekNo) {
    getDocs(refCollectUsers).then(
      (response) => {
        var users = response.docs.map((doc) => ({ ...doc.data(), id: doc.id }));
        var totalUsers = users.length;
        var getDayNumber = 0;
        getDocs(refCollectWeeklyAverage).then(
          (respond) => {
            var weekly = respond.docs.map((doc) => ({
              ...doc.data(),
              id: doc.id,
            }));
  ...
  const handleBarCodeScanned = async ({ data }) => {
    ...
    let time = hours + ":" + minutes; //Timestamp.fromDate(new Date());
    console.log(time);
    ...
    for (var countAtt = 0; countAtt < allAttendance.length; countAtt++) {
      if (
        allAttendance[countAtt].email === email &&
      ...
      <View style={styles.modal}>
        <View style={styles.innerModal}>
          <Text style={styles.modalHeader}>
            By Clicking Yes your are confirming that you logout
          </Text>

          <View style={styles.buttons}>
            <TouchableOpacity
              style={styles.yesClockOut}
              onPress={() => clockOut()}
            >
              <Text style={styles.clockOutText}>Yes</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.NoClockOut}
              onPress={() => setClockOutPopUp(false)}
            >
